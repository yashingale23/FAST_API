...
def delee(id: int, db: Session = Depends(get_db), current_user: schemas.Users = Depends(oauth2.get_current_user)):
    return blog_repo.destroy(id, db)

@router.put('/{id}', status_code=status.HTTP_202_ACCEPTED)
def update(id: int, request: schemas.blog, db: Session = Depends(get_db), current_user: schemas.Users = Depends(oauth2.get_current_user)):
    return blog_repo.update(id, request, db)

@router.get('/{id}', status_code=200, response_model=schemas.Showblog)
def shows(id: int, db: Session = Depends(get_db), current_user: schemas.Users = Depends(oauth2.get_current_user)):
